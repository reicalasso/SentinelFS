...
public:
    ...
    MockStorage() {
        if (sqlite3_open(":memory:", &db) != SQLITE_OK) {
            std::cerr << "Failed to open in-memory DB" << std::endl;
        }
        // Create tables needed for FileSyncHandler
    ...
    bool addFile(const std::string& path, const std::string& hash, long long timestamp, long long size) override {
        ...
        // Also insert into SQLite for FileSyncHandler
        const char* sql = "INSERT OR REPLACE INTO files (path, hash, size, timestamp) VALUES (?, ?, ?, ?)";
        sqlite3_stmt* stmt;
